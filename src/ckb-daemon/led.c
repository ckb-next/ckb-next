...
void initrgb(keylight* light){
    ...
    light->rgb = malloc(RGB_SIZE);
    light->enabled = 1;
    memset(light->rgb, 0xFF, RGB_SIZE);
}
...
void updateleds(usbdevice* kb){
    ...
    if(profile->light.enabled){
        const short* rgb = profile->light.rgb;
        for(int i = 0; i < 60; i++)
