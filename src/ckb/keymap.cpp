...
KeyMap KeyMap::standard(KeyMap::Model model, KeyMap::Layout layout){
    if(model == K70){
        ...
        if(layout == US)
            return KeyMap(model, layout, KEYCOUNT_K70_US, K70_WIDTH, K70PosUS);
        else
            return KeyMap(model, layout, KEYCOUNT_K70_UK, K70_WIDTH, K70PosUK);
    }
    // K95
    if(layout == US)
        return KeyMap(model, layout, KEYCOUNT_K95_US, K95_WIDTH, K95PosUS);
    else
        return KeyMap(model, layout, KEYCOUNT_K95_UK, K95_WIDTH, K95PosUK);
}
