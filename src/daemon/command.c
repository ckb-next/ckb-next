    ...
    while (sscanf(line, "%s%n", word, &wordlen) == 1)
    {
        ...
        for (int i = 0; i < CMD_COUNT - 1; i++)
        {
            if (!strcmp(word, cmd_strings[i]))
            {
                ...
                if (command != SWITCH && command != HWLOAD && command != HWSAVE && command != ACTIVE && command != IDLE && command != ERASE && command != ERASEPROFILE)
                    goto next_loop;
                break;
            }
        }
        ...
        if (command == NONE || ((!HAS_FEATURES(kb, FEAT_BIND) && (command == BIND || command == UNBIND || command == REBIND || command == MACRO || command == DELAY)) || (!HAS_FEATURES(kb, FEAT_NOTIFY) && command == NOTIFY)))
        {
        next_loop:
            continue;
        }
        ...
        while (position < left && sscanf(word + position, "%10[^:,]%n", keyname, &field) == 1)
        {
            ...
            else if ((sscanf(keyname, "#%u", &keycode) && keycode < N_KEYS_EXTENDED) || (sscanf(keyname, "#x%x", &keycode) && keycode < N_KEYS_EXTENDED))
            {
