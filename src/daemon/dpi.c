...
int updatedpi(usbdevice* kb, int force)
{
    ...
    if (!force && !lastdpi->forceupdate && !newdpi->forceupdate && !memcmp(lastdpi, newdpi, sizeof(dpiset)))
        return 0;
    ...
    if (newdpi->current != lastdpi->current)
    {
        ...
        if ((lastdpi->enabled & 1 << newdpi->current) == 0)
        {
            ...
            // stages, use it.
            if (newdpi->enabled & 1 << newdpi->current && newdpi->enabled & 1 << lastdpi->current)
            {
        ...
        if (newdpi->x[newdpi->current] != lastdpi->x[newdpi->current] || newdpi->y[newdpi->current] != lastdpi->y[newdpi->current])
        {
...
int savedpi(usbdevice* kb, dpiset* dpi, lighting* light)
{
    ...
    // Send settings
    uchar data_pkt[4][MSG_SIZE] = { { CMD_SET, FIELD_MOUSE, MOUSE_DPIMASK, 1, dpi->enabled }, { CMD_SET, FIELD_MOUSE, MOUSE_DPI, 1, dpi->current }, { CMD_SET, FIELD_MOUSE, MOUSE_LIFT, 1, dpi->lift }, { CMD_SET, FIELD_MOUSE, MOUSE_SNAP, 1, dpi->snap, 0x05 } };
