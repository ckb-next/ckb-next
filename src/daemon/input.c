...
static void* play_macro(void* param)
{
    ...
    while (macro_pt_first() != pthread_self())
...
static void _cmd_macro(usbmode* mode, const char* keys, const char* assignment, usbdevice* kb)
{
    ...
    {
        uint keycode;
        if ((sscanf(keyname, "#%u", &keycode) && keycode < N_KEYS_INPUT) || (sscanf(keyname, "#x%x", &keycode) && keycode < N_KEYS_INPUT))
        {
    ...
    while (position < right && sscanf(assignment + position, "%23[^,]%n", keyname, &field) == 1)
    {
        ...
        {
            uint keycode;
            if ((sscanf(keyname + 1, "#%u", &keycode) && keycode < N_KEYS_INPUT) || (sscanf(keyname + 1, "#x%x", &keycode) && keycode < N_KEYS_INPUT))
            {
                // Set a key numerically
