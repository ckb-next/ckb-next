...
int os_inputopen(usbdevice* kb)
{
    ...
    kr = IOConnectCallStructMethod(kb->event, initialize_virtual_hid_keyboard, &properties, sizeof(vhid_properties), NULL, 0);
    if (kr != KERN_SUCCESS)
    {
    ...
    while (1)
    {
        ...
        kr = IOConnectCallStructMethod(kb->event, is_virtual_hid_keyboard_ready, NULL, 0, &ready, &readysize);
    ...
    kr = IOConnectCallStructMethod(kb->event_mouse, initialize_virtual_hid_pointing, NULL, 0, NULL, 0);
    if (kr != KERN_SUCCESS)
