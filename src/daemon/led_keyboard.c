...
static int update_sidelights(usbdevice* kb)
{
    ...
    uchar data_pkt[MSG_SIZE] = { CMD_SET, FIELD_LIGHTING, MODE_SIDELIGHT, 0, !!newlight->sidelight, 0x00 };
    if (!usbsend(kb, data_pkt, 1))
        return -1;
    return 0;
}

static void makergb_512(const lighting* light, uchar data_pkt[5][MSG_SIZE], uchar (*ditherfn)(int, uchar))
{
...
int updatergb_kb(usbdevice* kb, int force)
{
    ...
    if (!force && !lastlight->forceupdate && !newlight->forceupdate && !rgbcmp(lastlight, newlight) && lastlight->sidelight == newlight->sidelight) // strafe sidelights
        return 0;
    ...
    if (kb->product == P_K66 || kb->product == P_K68_NRGB)
    {
        ...
        uchar winlock_pkt[MSG_SIZE] = { CMD_SET, FIELD_LIGHTING, MODE_WINLOCK, 0, winlock_status, 0 };
    ...
    else if (IS_FULLRANGE(kb))
    {
        ...
        uchar data_pkt[12][MSG_SIZE] = { // Red
    ...
    else
    {
        ...
        uchar data_pkt[5][MSG_SIZE] = { { CMD_WRITE_BULK, 0x01, 60, 0 }, { CMD_WRITE_BULK, 0x02, 60, 0 }, { CMD_WRITE_BULK, 0x03, 60, 0 }, { CMD_WRITE_BULK, 0x04, 36, 0 }, { CMD_SET, FIELD_KB_9BCLR, 0x00, 0x00, 0xD8 } };
...
int savergb_kb(usbdevice* kb, lighting* light, int mode)
{
    ...
    {
        uchar data_pkt[5][MSG_SIZE] = { { CMD_WRITE_BULK, 0x01, 60, 0 }, { CMD_WRITE_BULK, 0x02, 60, 0 }, { CMD_WRITE_BULK, 0x03, 60, 0 }, { CMD_WRITE_BULK, 0x04, 36, 0 }, { CMD_SET, FIELD_KB_HWCLR, 0x02, 0x00, 0x01, mode + 1 } };
    ...
    if (kb->fwversion >= 0x0120 || IS_V2_OVERRIDE(kb))
    {
        ...
        for (int clr = 0; clr < 3; clr++)
        {
            for (int i = 0; i < 4; i++)
            {
                ...
                if ((kb->fwversion >= 0x205) || ((kb->fwversion >= 0x204) && ((kb->product == P_K70_LUX_NRGB) || (kb->product == P_K70_LUX))))
                {
                ...
                if (memcmp(in_pkt[i], comparePacket, 4))
                {
                    ...
                    ckb_err("color = %d, i = %d, mode = %d\nOutput (Request): %2.2x %2.2x %2.2x %2.2x\nInput(Reply): %2.2x %2.2x %2.2x %2.2x %2.2x "
                            "%2.2x %2.2x %2.2x\n",
                            clr, i, mode, comparePacket[0], comparePacket[1], comparePacket[2], comparePacket[3], in_pkt[i][0], in_pkt[i][1], in_pkt[i][2], in_pkt[i][3], in_pkt[i][4], in_pkt[i][5], in_pkt[i][6], in_pkt[i][7]);
                    in_pkt[2][0] = 0x99;
                    in_pkt[2][1] = 0x99;
                    in_pkt[2][2] = 0x99;
                    in_pkt[2][3] = 0x99;
