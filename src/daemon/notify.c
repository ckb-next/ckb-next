...
static void _cmd_get(usbdevice* kb, usbmode* mode, int nnumber, const char* setting)
{
    ...
    else if (!strcmp(setting, ":i"))
    {
        ...
        if (kb->hw_ileds & I_NUM)
            nprintind(kb, nnumber, I_NUM, 1);
        if (kb->hw_ileds & I_CAPS)
            nprintind(kb, nnumber, I_CAPS, 1);
        if (kb->hw_ileds & I_SCROLL)
            nprintind(kb, nnumber, I_SCROLL, 1);
    }
