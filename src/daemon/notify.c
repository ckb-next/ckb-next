...
void _vnprintf(HANDLE fifo, const char* format, va_list va_args){
    ...
    if(!success) // TODO: Find a way to nuke the notify node on failure
        ckb_err("Couldn't send notification: %d, writen %lu, out: %s, err %lu\n", success, written, buf, GetLastError());
