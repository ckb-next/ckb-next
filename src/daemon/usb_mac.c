...
#define wait_loop(error, operation)                                                                 \
    do                                                                                              \
    {                                                                                               \
        int trial = 0;                                                                              \
...
#define IS_DISCONNECT_FAILURE(res) \
    ((res) == kIOReturnBadArgument || (res) == kIOReturnNoDevice || (res) == kIOReturnNotOpen || (res) == kIOReturnNotAttached || (res) == kIOReturnExclusiveAccess)
...
void* os_inputmain(void* context)
{
    ...
    CFRunLoopTimerRef krtimer = kb->krtimer = CFRunLoopTimerCreate(kCFAllocatorDefault, CFAbsoluteTimeGetCurrent() + 0.001, 0.001, // Set it to run every 1ms
                                                                   0, 0, keyretrigger, &krctx);
...
static usbdevice* add_hid(hid_dev_t handle, io_object_t** rm_notify)
{
    ...
    else
    {
        ...
        // Handle 2 is for Corsair inputs, unused on non-RGB
                 ...
                 && (((input == 64 || input == 15) && output == 0) || (input == 64 && output == 64) || // FW >= 1.20
        ...
        else if (output <= 1 && feature <= 1 && (input == 21 || input == 10 || input == 4 || input == 64)) // FW >= 2.00 (Scimitar)
            handle_idx = 1;
        else
        {
...
int usbmain()
{
    ...
    CFRunLoopTimerRef rmetimer = CFRunLoopTimerCreate(kCFAllocatorDefault, CFAbsoluteTimeGetCurrent() + 5, 5, // Set it to run every 5s
                                                      0, 0, register_mouse_event_tap, &rmectx);
    ...
    CFSocketRef cf_socket = CFSocketCreateWithNative(NULL, sighandler_pipe[SIGHANDLER_RECEIVER], kCFSocketReadCallBack, mac_exithandler, NULL);
