...
// Class for tracking an animation script. Has a global list of all possible scripts, and can also provide instances to launch the process and
// communicate with it. See also: KbAnim, KbLight
...
public:
    ...
    inline Param param(const QString& name) const
    {
        QListIterator<Param> i(_info.params);
        while (i.hasNext())
        {
            Param p = i.next();
            if (p.name == name)
        ...
        return ((Param[]) { { Param::INVALID, "", "", "", 0, 0, 0 } })[0];
    }
    inline bool hasParam(const QString& name) const
    {
        QListIterator<Param> i(_info.params);
        while (i.hasNext())
        {
