...
AnimSettingDialog::AnimSettingDialog(QWidget* parent, KbAnim* anim)
    : QDialog(parent), ui(new Ui::AnimSettingDialog), stopCheck(0), kpStopCheck(0), _anim(anim), lastDuration(1.0)
{
    ui->setupUi(this);
    ...
    while (i.hasNext())
    {
        ...
        if (param.name == "duration" || param.name == "trigger" || param.name == "kptrigger" || param.name == "kpmode" || param.name == "delay" || param.name == "kpdelay" || param.name == "repeat" || param.name == "kprepeat" || param.name == "stop" || param.name == "kpstop" || param.name == "kpmodestop" || param.name == "kprelease")
            continue;
        ...
        switch (param.type)
        {
            case AnimScript::Param::BOOL:
                ...
                colSpan = 4;
                connect((QCheckBox*)widget, &QCheckBox::stateChanged, [=]() { emit updateParam(param.name); });
                break;
            case AnimScript::Param::LONG:
                ...
                connect((QSpinBox*)widget, OVERLOAD_PTR(int, QSpinBox, valueChanged), [=]() { emit updateParam(param.name); });
                break;
            case AnimScript::Param::DOUBLE:
                ...
                connect((QDoubleSpinBox*)widget, OVERLOAD_PTR(double, QDoubleSpinBox, valueChanged), [=]() { emit updateParam(param.name); });
                break;
            case AnimScript::Param::RGB:
                widget = new ColorButton(this);
                ((ColorButton*)widget)->color(QColor("#" + value.toString()));
                colSpan = 3;
                connect((ColorButton*)widget, &ColorButton::colorChanged, [=]() { emit updateParam(param.name); });
                break;
            ...
            case AnimScript::Param::GRADIENT:
                ...
                colSpan = 3;
                connect((GradientButton*)widget, &GradientButton::gradientChanged, [=]() { emit updateParam(param.name); });
                break;
            ...
            case AnimScript::Param::ANGLE:
                ...
                connect((QDial*)widget, &QDial::valueChanged, [=]() { emit angleDialChanged(param.name); });
                break;
            case AnimScript::Param::STRING:
                widget = new QLineEdit(this);
                ((QLineEdit*)widget)->setText(value.toString());
                colSpan = 3;
                connect((QLineEdit*)widget, &QLineEdit::textEdited, [=]() { emit updateParam(param.name); });
                break;
        ...
        else
        {
            ...
            if (param.type == AnimScript::Param::ANGLE)
            {
                ...
                connect(spinner, OVERLOAD_PTR(int, QSpinBox, valueChanged), [=]() { emit angleSpinnerChanged(param.name); });
    ...
    connect(check, &QCheckBox::stateChanged, [=]() { emit updateParam("trigger"); });
    row++;
    ...
    connect(check, &QCheckBox::stateChanged, [=]() { emit updateParam("kptrigger"); });
    ...
    connect(combo, OVERLOAD_PTR(int, QComboBox, activated), [=]() { emit updateParam("kpmode"); });
    row++;
    ...
    connect(ui->kpModeStopBox, &QCheckBox::clicked, [=]() { emit updateParam("kpmodestop"); });
    ...
    if (anim->hasParameter("repeat"))
    {
        ...
        connect(spinner, OVERLOAD_PTR(int, QSpinBox, valueChanged), [=]() { emit updateParam("stop"); });
        ...
        connect(spinner, OVERLOAD_PTR(int, QSpinBox, valueChanged), [=]() { emit updateParam("kpstop"); });
    ...
    else
    {
        ...
        settingWidgets["stop"] = spinner;
        connect(spinner, OVERLOAD_PTR(double, QDoubleSpinBox, valueChanged), [=]() { emit updateParam("stop"); });
        ui->timeGrid->addWidget(spinner, 4, 3);
        ...
        settingWidgets["kpstop"] = spinner;
        connect(spinner, OVERLOAD_PTR(double, QDoubleSpinBox, valueChanged), [=]() { emit updateParam("kpstop"); });
        ui->timeGrid->addWidget(spinner, 12, 3);
...
void AnimSettingDialog::updateParam(QString name)
{
    ...
    if ((name == "stop" && stopCheck->isChecked() == hasRepeat) || (name == "kpstop" && kpStopCheck->isChecked() == hasRepeat))
    {
