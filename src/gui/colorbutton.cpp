...
ColorButton::ColorButton(QWidget* parent, bool allowAlpha) : QPushButton(parent), _alpha(allowAlpha), _setLabel(true), _bigIcons(false)
{
...
void ColorButton::versionWarning()
{
    ...
    if (!settings.value("QtWarningDismissed").toBool())
    {
        ...
        // Whitelist
        QStringList qpaThemeWhiteList = QStringList() << "qt5ct"
                                                      << "kde"
                                                      << "appmenu-qt5";
        if (qpaTheme.isEmpty() || !qpaThemeWhiteList.contains(qpaTheme))
        {
            ...
            qtWarning.setText(
                tr("A system configuration that can lead to instability issues with this software has been detected.<br><br>"
                   "If this application locks up after clicking the OK button below, please refer to "
                   "<a "
                   "href=\"https://github.com/ckb-next/ckb-next/wiki/Troubleshooting#glib-critical-errors-and-lock-up-at-colour-chooser\">https://"
                   "github.com/ckb-next/ckb-next/wiki/Troubleshooting#glib-critical-errors-and-lock-up-at-colour-chooser</a>"));
