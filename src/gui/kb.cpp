    ...
    cmd(cmdpath), notifyNumber(1), macroNumber(2), _needsSave(false), _layout(KeyMap::NO_LAYOUT), _maxDpi(0)
{
    ...
    if (mpath.open(QIODevice::ReadOnly)){
        ...
        usbModel = usbModel.remove("Corsair", Qt::CaseInsensitive).remove("Gaming").remove("Keyboard").remove("Mouse").remove("Bootloader").remove("Mechanical").replace("LOW PROFILE", "LP").remove("Headset").remove("USB").trimmed();
        mpath.close();
    }
    if (usbModel == "")
        usbModel = "Keyboard";
