...
KbAnim::KbAnim(QObject* parent, const KeyMap& map, const QUuid id, CkbSettings& settings)
    : QObject(parent), _script(0), _map(map), repeatTime(0), kpRepeatTime(0), stopTime(0), kpStopTime(0), repeatMsec(0), kpRepeatMsec(0), _guid(id), _isActive(false), _isActiveKp(false), _needsSave(false)
{
...
KbAnim::KbAnim(QObject* parent, const KeyMap& map, const QString& name, const QStringList& keys, const AnimScript* script)
    : QObject(parent), _script(AnimScript::copy(this, script->guid())), _map(map), _keys(keys), repeatTime(0), kpRepeatTime(0), stopTime(0), kpStopTime(0), repeatMsec(0), kpRepeatMsec(0), _guid(QUuid::createUuid()), _name(name), _opacity(1.), _mode(Normal), _isActive(false), _isActiveKp(false), _needsSave(true)
{
    if (_script)
    {
...
KbAnim::KbAnim(QObject* parent, const KeyMap& map, const KbAnim& other)
    : QObject(parent), _script(AnimScript::copy(this, other.script()->guid())), _scriptGuid(_script->guid()), _scriptName(_script->name()), _map(map),
      _keys(other._keys), _parameters(other._parameters), repeatTime(0), kpRepeatTime(0), stopTime(0), kpStopTime(0), repeatMsec(0), kpRepeatMsec(0),
