...
KbBind::KbBind(KbMode* modeParent, Kb* parentBoard, const KeyMap& keyMap)
    : QObject(modeParent), _devParent(parentBoard), lastGlobalRemapTime(globalRemapTime), _map(keyMap), _winLock(false), _needsUpdate(true), _needsSave(true)
{}
...
KbBind::KbBind(KbMode* modeParent, Kb* parentBoard, const KeyMap& keyMap, const KbBind& other)
    : QObject(modeParent), _devParent(parentBoard), lastGlobalRemapTime(globalRemapTime), _bind(other._bind), _winLock(false), _needsUpdate(true), _needsSave(true)
{
    map(keyMap);
...
KeyAction* KbBind::bindAction(const QString& key)
{
    if (!_bind.contains(key))
        return _bind[key] = new KeyAction(KeyAction::defaultAction(key, devParent()->model()), this);
    return _bind[key];
}
...
void KbBind::update(QFile& cmd, int notify, bool force)
{
    ...
    if (!_bind.contains("caps"))
        bind["caps"] = 0;
    if (!_bind.contains("lshift"))
        bind["lshift"] = 0;
    if (!_bind.contains("rshift"))
        bind["rshift"] = 0;
    if (!_bind.contains("lctrl"))
        bind["lctrl"] = 0;
    if (!_bind.contains("rctrl"))
        bind["rctrl"] = 0;
    if (!_bind.contains("lwin"))
        bind["lwin"] = 0;
    if (!_bind.contains("rwin"))
        bind["rwin"] = 0;
    if (!_bind.contains("lalt"))
        bind["lalt"] = 0;
    if (!_bind.contains("ralt"))
        bind["ralt"] = 0;
    if (!_bind.contains("fn"))
        bind["fn"] = 0;
...
void KbBind::handleNotificationChannel(bool start)
{
    if (getMacroNumber() > 0 && lastCmd)
    {
        ...
        qDebug() << QString("No cmd or valid handle for notification found, macroNumber = %1, lastCmd = %2").arg(getMacroNumber()).arg(lastCmd ? "set" : "unset");
}
