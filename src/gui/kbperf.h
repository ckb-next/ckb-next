...
public:
    ...
    inline QPoint dpi(int index) const
    {
        if (index < 0 || index >= DPI_COUNT)
            return QPoint();
        return QPoint(dpiX[index], dpiY[index]);
    }
    ...
    inline void dpiEnabled(int index, bool newEnabled)
    {
        if (index <= 0)
            return;
        dpiOn[index] = newEnabled;
    ...
    inline void iOpacity(float newIOpacity)
    {
        _iOpacity = newIOpacity;
        _needsSave = true;
    }
    // DPI indicator colors
    inline bool dpiIndicator() const { return _dpiIndicator; }
    inline void dpiIndicator(bool newDpiIndicator)
    {
        _dpiIndicator = newDpiIndicator;
        _needsSave = true;
    }
    const static int OTHER = DPI_COUNT; // valid only with dpiColor
    inline QColor dpiColor(int index) const { return dpiClr[index]; }
    inline void dpiColor(int index, const QColor& newColor)
    {
        dpiClr[index] = newColor;
        _needsUpdate = _needsSave = true;
    }
    // KB indicator colors
    enum indicator
