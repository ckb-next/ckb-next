...
public:
    ...
    {
        _needsSave = true;
        _name = newName.trimmed();
        if (_name == "")
            _name = "Unnamed";
    }
    inline UsbId& id() { return _id; }
    inline void id(const UsbId& newId)
    {
        _needsSave = true;
        _id = newId;
    }
    ...
    inline void modes(const QList<KbMode*>& newModes)
    {
        setNeedsUpdate();
        _modes = newModes;
    }
    inline void append(KbMode* newMode)
    {
        setNeedsUpdate();
        _modes.append(newMode);
    }
    inline void insert(int index, KbMode* newMode)
    {
        setNeedsUpdate();
        _modes.insert(index, newMode);
    }
    inline void removeAll(KbMode* mode)
    {
        setNeedsUpdate();
        _modes.removeAll(mode);
    }
    ...
    inline KbMode* find(const QUuid& id)
    {
    ...
    inline void currentMode(KbMode* newCurrentMode)
    {
        _needsSave = true;
