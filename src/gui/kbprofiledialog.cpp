...
KbProfileDialog::KbProfileDialog(KbWidget* parent) : QDialog(parent), ui(new Ui::KbProfileDialog), device(parent->device)
{
    ui->setupUi(this);
...
void KbProfileDialog::repopulate()
{
    ...
    foreach (KbProfile* profile, device->profiles())
    {
        QListWidgetItem* item
            = new QListWidgetItem(QIcon((profile == device->hwProfile()) ? ":/img/icon_profile_hardware.png" : ":/img/icon_profile.png"), profile->name(), ui->profileList);
...
void KbProfileDialog::on_importButton_clicked()
{
...

    {
        ...
        if (device->isKeyboard() != metadata.value("isKeyboard").toBool() || device->isMouse() != metadata.value("isMouse").toBool()
            || device->isMousepad() != metadata.value("isMousepad").toBool() || device->isHeadsetStand() != metadata.value("isHeadsetStand").toBool())
        {
        ...
        if (currentDevModel != metadataDevModel || currentDevLayout != metadataDevLayout)
        {
            ...
            int ret = QMessageBox::question(this, tr("Profile Import"),
                                            tr("This profile was created for a %1 (%2)\nbut it is going to be imported to a %3 (%4).\n\n"
                                               ...
                                               "Import Anyway?")
                                                .arg(metadataDevModel, metadataDevLayoutStr, currentDevModel, currentDevLayoutStr),
    ...
    int ret = QMessageBox::information(this, tr("Profile Import"), tr("The following profiles will be imported.\n\n• ").append(profilestrf), QMessageBox::Ok, QMessageBox::Cancel);
    ...
    for (int i = 0; i < profileptrs.count(); i++)
    {
        ...
        foreach (KbProfile* profile, device->profiles())
        {
            ...
            {
                ret = QMessageBox::question(this, tr("Profile Import"), profname + tr(" already exists. Overwrite it?"), QMessageBox::Yes | QMessageBox::No | QMessageBox::Cancel, QMessageBox::Cancel);
