...
KeyAction::KeyAction(const QString& action, QObject* parent) : QObject(parent), _value(action), preProgram(0), relProgram(0), sniperValue(0) {}

KeyAction::KeyAction(QObject* parent) : QObject(parent), _value(""), preProgram(0), relProgram(0), sniperValue(0) {}
...
QString KeyAction::defaultAction(const QString& key, KeyMap::Model model)
{
    ...
    if (key.length() >= 2 && key[0] == 'g' && ((key.length() == 2 && key[1] >= '0' && key[1] <= '9') || (key.length() == 3 && key[1] == '1' && key[2] >= '0' && key[2] <= '8')))
        return "";
...
QString KeyAction::programAction(const QString& onPress, const QString& onRelease, int stop)
{
    ...
    return "$program:" + QString::fromUtf8(QUrl::toPercentEncoding(onPress.trimmed())) + "+" + QString::fromUtf8(QUrl::toPercentEncoding(onRelease.trimmed()))
           + QString("+%1").arg(stop);
}
...
void KeyAction::keyEvent(KbBind* bind, bool down)
{
    ...
    else if (prefix == "$dpi")
    {
        ...
        switch (level)
        {
            ...
            default:
                if (level < 1 || level >= KbPerf::DPI_COUNT || !down)
                    return;
    ...
    else if (prefix == "$program")
    {
        ...
        if ((down && (stop & PROGRAM_PR_MULTI)) || (!down && (stop & PROGRAM_RE_MULTI)))
        {
