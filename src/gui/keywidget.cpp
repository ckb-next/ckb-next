...
void KeyWidget::paintEvent(QPaintEvent*)
{
    ...
    while (k.hasNext())
    {
        ...
        if ((model != KeyMap::STRAFE && model != KeyMap::K95P && model != KeyMap::K70MK2 && model != KeyMap::STRAFE_MK2) && (!strcmp(key.name, "mr") || !strcmp(key.name, "m1") || !strcmp(key.name, "m2") || !strcmp(key.name, "m3") || !strcmp(key.name, "light") || !strcmp(key.name, "lock") || (model == KeyMap::K65 && !strcmp(key.name, "mute"))))
        {
        ...
        while (k.hasNext())
        {
            ...
            struct
            {
                const char *keyName, *displayName;
                          ...
                          { "lock", "☒" },
                          { "mute", "◖⊘" },
                          { "volup", keyMap.model() == KeyMap::K65 ? "◖))" : "▲" },
                          { "voldn", keyMap.model() == KeyMap::K65 ? "◖)" : "▼" },
                          { "prtscn", "PrtScn\nSysRq" },
                          { "scroll", "Scroll\nLock" },
                          { "pause", "Pause\nBreak" },
                          { "stop", "▪" },
...
{
    event->accept();
    mouseDownMode = (event->modifiers() & Qt::AltModifier)     ? SUBTRACT :
                    (event->modifiers() & Qt::ShiftModifier)   ? ADD :
                    (event->modifiers() & Qt::ControlModifier) ? TOGGLE :
                                                                 SET;
...
void KeyWidget::mouseMoveEvent(QMouseEvent* event)
{
    ...
    while (k.hasNext())
    {
        ...
        if (fabs(key.x - mx) <= key.width / 2.f - 1.f && fabs(key.y - my) <= key.height / 2.f - 1.f && tooltip.isEmpty())
            tooltip = key.friendlyName(false);
        // on STRAFE Sidelights and indicators can't be assigned color the way other keys are colored
        if (((keyMap.model() == KeyMap::STRAFE || keyMap.model() == KeyMap::STRAFE_MK2) && (!strcmp(key.name, "lsidel") || !strcmp(key.name, "rsidel"))) || (keyMap.model() == KeyMap::M95 && !strcmp(key.name, "back"))
        ...
        if (!(mx1 >= kx2 || kx1 >= mx2) && !(my1 >= ky2 || ky1 >= my2) && mouseDownMode != NONE)
            newSelection.setBit(i);
    }
...
void KeyWidget::selectAll()
{
    ...
    foreach (const Key& key, keyMap.positions())
    {
        ...
        if (strcmp(key.name, "lsidel") && strcmp(key.name, "rsidel") && keyMap.model() != KeyMap::M95 && ((_rgbMode && key.hasLed) || !(_rgbMode && key.hasScan)))
        {
            selection.setBit(i);
