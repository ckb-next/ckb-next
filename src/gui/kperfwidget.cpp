...
/// \param parent
///
KPerfWidget::KPerfWidget(QWidget* parent) : QWidget(parent), ui(new Ui::KPerfWidget)
{
    ui->setupUi(this);
    // Set up indicators
    indicators[0].enable = 0;
    indicators[0].hwEnable = ui->numBox;
    indicators[0].color1 = ui->numColorOn;
    indicators[0].color2 = ui->numColorOff;
    indicators[0].color3 = 0;
    indicators[1].enable = 0;
    indicators[1].hwEnable = ui->capsBox;
    indicators[1].color1 = ui->capsColorOn;
    indicators[1].color2 = ui->capsColorOff;
    indicators[1].color3 = 0;
    indicators[2].enable = 0;
    indicators[2].hwEnable = ui->scrollBox;
    indicators[2].color1 = ui->scrollColorOn;
    indicators[2].color2 = ui->scrollColorOff;
    indicators[2].color3 = 0;
    indicators[3].enable = ui->modeBox;
    indicators[3].hwEnable = 0;
    indicators[3].color1 = ui->modeColorOn;
    indicators[3].color2 = ui->modeColorOff;
    indicators[3].color3 = 0;
    indicators[4].enable = ui->macroBox;
    indicators[4].hwEnable = 0;
    indicators[4].color1 = ui->macroColorOn;
    indicators[4].color2 = ui->macroColorOff;
    indicators[4].color3 = 0;
    indicators[5].enable = ui->lightBox;
    indicators[5].hwEnable = 0;
    indicators[5].color1 = ui->lightColor1;
    indicators[5].color2 = ui->lightColor2;
    indicators[5].color3 = ui->lightColor3;
    indicators[6].enable = ui->lockBox;
    indicators[6].hwEnable = 0;
    indicators[6].color1 = ui->lockColorOn;
    indicators[6].color2 = ui->lockColorOff;
    indicators[6].color3 = 0;
    indicators[7].enable = ui->muteBox;
    indicators[7].hwEnable = 0;
    indicators[7].color1 = ui->muteColorOn;
    indicators[7].color2 = ui->muteColorOff;
    indicators[7].color3 = ui->muteColorNA;
    for (int i = 0; i < I_COUNT; i++)
    {
        ...
        {
            connect(indicators[i].enable, &QCheckBox::clicked, [=]() { emit uiUpdated(i); });
        }
        if (indicators[i].hwEnable)
        {
            connect(indicators[i].hwEnable, OVERLOAD_PTR(int, QComboBox, activated), [=]() { emit uiUpdated(i); });
        }
        connect(indicators[i].color1, &ColorButton::colorChanged, [=]() { emit uiUpdated(i); });
        connect(indicators[i].color2, &ColorButton::colorChanged, [=]() { emit uiUpdated(i); });
        if (indicators[i].color3)
        {
            connect(indicators[i].color3, &ColorButton::colorChanged, [=]() { emit uiUpdated(i); });
        }
    }
...
void KPerfWidget::setPerf(KbPerf* newPerf, KbProfile* newProfile)
{
    ...
    for (int i = HW_I_COUNT; i < I_COUNT; i++)
    {
        ...
        indicators[i].color2->color(c2);
        if (indicators[i].color3)
            indicators[i].color3->color(c3);
        if (software)
        {
            indicators[i].color1->setEnabled(true);
            indicators[i].color2->setEnabled(true);
            if (indicators[i].color3)
                indicators[i].color3->setEnabled(true);
        }
        else
        {
            indicators[i].color1->setEnabled(false);
            indicators[i].color2->setEnabled(false);
            if (indicators[i].color3)
                indicators[i].color3->setEnabled(false);
        }
    }
    ...
    else
    {
        foreach (QWidget* w, k95Widgets)
            w->hide();
        if (profile->keyMap().model() == KeyMap::STRAFE)
        { // hide indicators unsupported by STRAFE
            ui->muteBox->hide();
            ui->muteColorOn->hide();
...
void KPerfWidget::uiUpdated(int index)
{
    ...
    if (software)
    {
        ...
        if (indicators[index].color3)
            indicators[index].color3->setEnabled(true);
    }
    else
    {
        ...
        if (indicators[index].color3)
            indicators[index].color3->setEnabled(false);
    }
