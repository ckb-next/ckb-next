    ...
    : QWidget(parent), ui(new Ui::MPerfWidget), perf(0), profile(0), _xyLink(!CkbSettings::get(xyLinkPath).toBool()), colorLink(false), isSetting(false)
{
    ...
    // Set up DPI stages
    stages[0].indicator = ui->iButton0;
    stages[0].xSlider = ui->xSlider0;
    stages[0].ySlider = ui->ySlider0;
    stages[0].xBox = ui->xBox0;
    stages[0].yBox = ui->yBox0;
    stages[0].enableCheck = 0;
    stages[1].indicator = ui->iButton1;
    stages[1].xSlider = ui->xSlider1;
    stages[1].ySlider = ui->ySlider1;
    stages[1].xBox = ui->xBox1;
    stages[1].yBox = ui->yBox1;
    stages[1].enableCheck = ui->eBox1;
    stages[2].indicator = ui->iButton2;
    stages[2].xSlider = ui->xSlider2;
    stages[2].ySlider = ui->ySlider2;
    stages[2].xBox = ui->xBox2;
    stages[2].yBox = ui->yBox2;
    stages[2].enableCheck = ui->eBox2;
    stages[3].indicator = ui->iButton3;
    stages[3].xSlider = ui->xSlider3;
    stages[3].ySlider = ui->ySlider3;
    stages[3].xBox = ui->xBox3;
    stages[3].yBox = ui->yBox3;
    stages[3].enableCheck = ui->eBox3;
    stages[4].indicator = ui->iButton4;
    stages[4].xSlider = ui->xSlider4;
    stages[4].ySlider = ui->ySlider4;
    stages[4].xBox = ui->xBox4;
    stages[4].yBox = ui->yBox4;
    stages[4].enableCheck = ui->eBox4;
    stages[5].indicator = ui->iButton5;
    stages[5].xSlider = ui->xSlider5;
    stages[5].ySlider = ui->ySlider5;
    stages[5].xBox = ui->xBox5;
    stages[5].yBox = ui->yBox5;
    stages[5].enableCheck = ui->eBox5;
    ...
    for (int i = 0; i < DPI_COUNT; i++)
    {
        ...
        // Map signals
        connect(stages[i].indicator, &ColorButton::clicked, [=]() { emit colorClicked(i); });
        connect(stages[i].indicator, &ColorButton::colorChanged, [=]() { emit colorChanged(i); });
        // valueChanged is used to update the settings
        // sliderMoved is used to update the spinbox in realtime
        connect(stages[i].xSlider, &QSlider::valueChanged, [=]() { emit sliderXValueChanged(i); });
        connect(stages[i].ySlider, &QSlider::valueChanged, [=]() { emit sliderYValueChanged(i); });
        connect(stages[i].xSlider, &QSlider::sliderMoved, [=]() { emit sliderXMoved(i); });
        connect(stages[i].ySlider, &QSlider::sliderMoved, [=]() { emit sliderYMoved(i); });
        connect(stages[i].xBox, OVERLOAD_PTR(int, QSpinBox, valueChanged), [=]() { emit boxXChanged(i); });
        connect(stages[i].yBox, OVERLOAD_PTR(int, QSpinBox, valueChanged), [=]() { emit boxYChanged(i); });
        if (stages[i].enableCheck)
            // Sniper has no enable
            connect(stages[i].enableCheck, &QCheckBox::stateChanged, [=]() { emit enableChanged(i); });
...
void MPerfWidget::setLegacyM95()
{
    ...
    QWidget* w[] = { ui->indicBox, ui->spinBox,

                     // Hide colour buttons
                     ui->iButton0, ui->iButton1, ui->iButton2, ui->iButton3, ui->iButton4, ui->iButton5, ui->iButtonO,

                     // Hide stages 4, 5, and other completely
                     ui->label_3, ui->label_15,

                     // Hide checkboxes as we can't disable DPI stages
                     ui->eBox1, ui->eBox2, ui->eBox3, ui->eBox4, ui->eBox5, ui->label_13, ui->label_8, ui->xSlider4, ui->xSlider5, ui->ySlider4, ui->ySlider5, ui->xBox4, ui->xBox5, ui->yBox5, ui->yBox4 };
