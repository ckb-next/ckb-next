...
RebindWidget::RebindWidget(QWidget* parent) : QWidget(parent), ui(new Ui::RebindWidget), bind(0), profile(0), macReader(0)
{
    ui->setupUi(this);
    ...
    // Add tip label
    ui->progTipLabel->setText(
        "Tip: use the open command to launch a file, directory, or app. For instance, to start Safari:\n  open /Applications/Safari.app");
#else
    ...
    else if (act.isSpecial())
    {
        ...
        else if (sAction == "macro")
        {
            ...
            if (act.isValidMacro())
            {
                ...
                if (act.macroTiming() != "x")
                    ui->txtBuffer->setText(act.macroTiming());
...
void RebindWidget::setBox(QWidget* box)
{
    ...
    // Mouse
    if (box != ui->mbBox)
        ui->mbBox->setCurrentIndex(0);
    if (box != ui->mb2Box)
        ui->mb2Box->setCurrentIndex(0);
    if (box != ui->wheelBox)
        ui->wheelBox->setCurrentIndex(0);
    if (box != ui->dpiBox)
        ui->dpiBox->setCurrentIndex(0);
    // Anim
    if (box != ui->animBox)
        ui->animBox->setCurrentIndex(0);
    // Special
    if (box != ui->modeBox)
        ui->modeBox->setCurrentIndex(0);
    if (box != ui->lightBox)
