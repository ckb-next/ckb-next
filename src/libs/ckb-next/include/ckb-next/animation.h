...
// Parameter helpers
#define CKB_PARAM(type, name, prefix, postfix, extra) \
...
#define CKB_PARAM_ARGB(name, prefix, postfix, a, r, g, b) \
...
#define CKB_PARSE_ARGB(param_name, a_ptr, r_ptr, g_ptr, b_ptr) \
...
#define CKB_KEYCLEAR(context)                                                                       \
    CKB_CONTAINER(ckb_key* key = context->keys; unsigned count = context->keycount; unsigned i = 0; \
                  for (; i < count; i++) key[i].a = key[i].r = key[i].g = key[i].b = 0;)
...
void urldecode(char* dst, const char* src)
{
    ...
    while ((s = *src))
    {
        if ((s == '%') && ((a = src[1]) && (b = src[2])) && (isxdigit(a) && isxdigit(b)))
        {
            if (a >= 'a')
                a -= 'a' - 'A';
            if (a >= 'A')
